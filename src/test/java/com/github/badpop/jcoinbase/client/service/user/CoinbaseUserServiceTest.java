...
class CoinbaseUserServiceTest {
  ...
  class FetchCurrentUserTest {
    ...
    void should_return_User() throws IOException {
      ...
      var actual = service.fetchCurrentUser(client, client.getAuthService());

      assertThat(actual).isSuccess().containsInstanceOf(User.class);
      Assertions.assertThat(actual.get())
          .usingRecursiveComparison()
          .ignoringFields("referralMoney.currencySymbol")
          // TODO SEARCH WHY CURRENCY SYMBOL DOES NOT WORK IN TESTS BUT WORK IN CODE
          .isEqualTo(
              User.builder()
                  .id("ID")
                  .name("name")
